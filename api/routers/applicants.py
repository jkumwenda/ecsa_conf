...
from models import Applicant, Country, Product, ProductLicenceStatus
...
async def get_applicant(
    ...
    products = (
        db.query(
            Product, ProductLicenceStatus.renewal_date, ProductLicenceStatus.entry_date
        )
        .join(ProductLicenceStatus, Product.id == ProductLicenceStatus.product_id)
        .filter(Product.applicant_id == applicant_id, ProductLicenceStatus.status == 1)
        .options(joinedload(Product.generic_name))
        .all()
    )
    ...
    products_data = [
        {
            ...
            "generic_name": product[0].generic_name.generic_name,
            "renewal_date": product[1],
            "entry_date": product[2],
        }
