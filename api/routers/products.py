...
import utils
from schemas.maladis import (
    ProductSchema,
    ProductApprovalSchema,
    ProductVariationSchema,
    ProductManufacturerSchema,
    ProductActiveIngredientSchema,
)
...
async def get_products(
    ...
    query = (
        ...
        .join(ProductLicenceStatus, Product.id == ProductLicenceStatus.product_id)
        .filter(
            or_(
                Product.product_name.ilike(f"%{search}%"),
                Product.product_id.ilike(f"%{search}%"),
            ),
            ProductLicenceStatus.status == 1,
        )
        .options(
            ...
            joinedload(Product.product_licence_status),
        )
        .offset(offset)
        .limit(limit)
        .all()
...
async def add_product(
    ...
    create_product_model = Product(
        ...
        markings=product_schema.markings,
        shape=product_schema.shape,
        unit_dose=product_schema.unit_dose,
    ...
    create_product_licence_status_model = ProductLicenceStatus(
        ...
        renewal_date=product_schema.renewal_date,
        status=1,
    )
...
async def get_product(
    ...
    product_licence_status = (
        ...
        .filter(ProductLicenceStatus.product_id == product.id)
        .first()
    )
    ...
    return {
        ...
        "product_licence_status": {
            ...
            "renewal_date": product_licence_status.renewal_date,
        ...
        "product_manufacturers": [
            {
                ...
                "manufacturer": product_manufacturer.manufacturer.manufacturer,
                "country": product_manufacturer.country.country,
            }
            for product_manufacturer in product.product_manufacturer
        ],
        "product_active_ingredients": [
            {
                "id": product_active_ingredient.id,
                "ingredient": product_active_ingredient.ingredient,
                "specification_number": product_active_ingredient.specification_number,
                "manufacturer": product_active_ingredient.manufacturer.manufacturer,
                "country": product_active_ingredient.country.country,
            }
            for product_active_ingredient in product.product_active_ingredient
        ],
        "product_variations": [
            {
                ...
                "created_at": product_variation.created_at,
            }
...
async def update_product(
    ...
    product_model.markings = product_schema.markings
    product_model.shape = product_schema.shape
    product_model.unit_dose = product_schema.unit_dose
    ...
    product_licence_status_model = (
        db.query(ProductLicenceStatus)
        .filter(
            ProductLicenceStatus.product_id == product_id,
            ProductLicenceStatus.status == 1,
        )
        .first()
    )

    product_licence_status_model.renewal_date = product_schema.renewal_date
    product_licence_status_model.entry_date = product_schema.entry_date
    db.commit()
...
async def generate_certificate_pdf(
    ...
    # Add product approvals
    pdf.setFont("Helvetica-Bold", 11)
    pdf.drawString(50, y_position, f"Valid until: {str(product.renewal_date)}")
    y_position -= 30

    # Add product approvals
    pdf.setFont("Helvetica-Bold", 11)
    pdf.drawString(50, y_position, f"Director general: Mphatso Kawaye")
    y_position -= 30

    logo_path = "assets/kawaye.jpg"  # Replace with the path to your logo image
    logo = ImageReader(logo_path)
    pdf.drawImage(logo, 50, y_position, width=90, height=30)  # Adjust position

    # Add product approvals
    pdf.setFont("Helvetica-Bold", 11)
    pdf.drawString(50, y_position, f"Chairperson: Frida Chimimba")
    y_position -= 10

    logo_path = "assets/kawaye.jpg"  # Replace with the path to your logo image
    logo = ImageReader(logo_path)
    pdf.drawImage(logo, 50, 7, width=90, height=30)  # Adjust position
...
@router.post("/add_product_manufacturer")
async def add_product_manufacturer(
    ...
    create_product_manufacturer_model = ProductManufacturer(
        product_id=product_manufacturer_schema.product_id,
        manufacturer_id=product_manufacturer_schema.manufacturer_id,
        country_id=product_manufacturer_schema.country_id,
    )
    db.add(create_product_manufacturer_model)
    db.commit()
    db.refresh(create_product_manufacturer_model)

    return product_manufacturer_schema


@router.post("/add_product_active_ingredient")
async def add_product_active_ingredient(
    product_active_ingredient_schema: ProductActiveIngredientSchema,
    user: user_dependency,
    db: Session = Depends(get_db),
):
    security.secureAccess("ADD_PRODUCT", user["id"], db)

    create_product_active_ingredient_model = ProductActiveIngredient(
        ingredient=product_active_ingredient_schema.ingredient,
        specification_number=product_active_ingredient_schema.specification_number,
        product_id=product_active_ingredient_schema.product_id,
        manufacturer_id=product_active_ingredient_schema.manufacturer_id,
        country_id=product_active_ingredient_schema.country_id,
    )
    db.add(create_product_active_ingredient_model)
    db.commit()
    db.refresh(create_product_active_ingredient_model)

    return product_active_ingredient_schema
...
async def add_product_variation(
    ...
    create_product_variation_model = ProductVariation(
        ...
        unit_dose=product_variation_schema.unit_dose,
        shape=product_variation_schema.shape,
        markings=product_variation_schema.markings,
