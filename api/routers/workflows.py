...
from schemas.maladis import ApprovalStageSchema, ApprovalStageSchema
...
async def get_approval_stages(
    ...
    query = (
        ...
        .filter(
            or_(
                ApprovalStage.stage.ilike(f"%{search}%"),
            )
        )
        .offset(offset)
        .limit(limit)
...
async def add_approval_stage(
    ...
    create_approval_stage_model = ApprovalStage(
        role_id=approval_stage_schema.role_id,
        stage=approval_stage_schema.stage,
        stage_number=approval_stage_schema.stage_number,
    )

    db.add(create_approval_stage_model)
    db.commit()
    return approval_stage_schema


@router.get("/{workflow_id}")
async def get_approval_stage(
    workflow_id: int,
    user: user_dependency,
    db: Session = Depends(get_db),
):
    ...
    workflow = get_object(workflow_id, db, ApprovalStage)
...
async def update_approval_stage(
    workflow_id: int,
    user: user_dependency,
    workflow_schema: ApprovalStageSchema,
    db: Session = Depends(get_db),
):
    security.secureAccess("UPDATE_WORKFLOW", user["id"], db)
    workflow_model = get_object(workflow_id, db, ApprovalStage)
...
async def delete_approval_stage(
    ...
    get_object(workflow_id, db, ApprovalStage)
    db.query(ApprovalStage).filter(ApprovalStage.id == workflow_id).delete()
    db.commit()
    raise HTTPException(
