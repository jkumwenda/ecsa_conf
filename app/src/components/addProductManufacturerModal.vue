        ...
        <div class="flex space-x-4">
          <label class="block w-6/12">
            ...
            <select name="manufacturer_id"
              ...
              v-model="productManufacturerData.manufacturer_id">
          ...
          <label class="block w-6/12">
            ...
            <select name="country_id"
              class="mt-2 px-3 py-3 bg-white border border-1 shadow-sm border-mid-gray-300 placeholder-slate-400 focus:outline-none focus:border-sky-500 focus:ring-sky-500 block w-full rounded-xl sm:text-sm focus:ring-1"
              v-model="productManufacturerData.country_id">
...
export default {
  ...
  data() {
    return {
      headerTitle: "Product Manufacturer Registration",
  ...
  watch: {
    product: {
      immediate: true,
      handler(newValue) {
        this.productManufacturerData.product_id = newValue.id
        this.getCountries();
  ...
  methods: {
    async addProductManufacturer() {
      this.isLoading = true;
      try {
        const response = await createItem("products/add_product_manufacturer/", this.productManufacturerData);
