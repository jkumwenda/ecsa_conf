        ...
        <div class="flex flex-col space-y-4">
            ...
            <div v-else class="rounded-2xl bg-catskill-white-100 shadow-sm p-4">
                <div class="flex flex-row bg-athens-gray-500 p-2 text-sm font-bold">
                    ...
                    <div class="sm:w-1/12">Renewal Date</div>
                    <div class="sm:w-1/12">Status</div>
                    <div class="sm:w-1/12">Action</div>
                </div>
                <div class="flex flex-row p-2 text-sm items-center" v-for="(product, index) in products"
                    :key="product.id" :class="getRowClass(index)">
                    ...
                    <div class="sm:w-1/12">{{ formatDate(product.product_licence_status['0']['entry_date']) }}</div>
                    <div class="sm:w-1/12">{{
                        formatDate(product.product_licence_status['0']['renewal_date']) }}</div>
                    <div class="sm:w-1/12">{{ formartStatus(product.product_licence_status['0']['renewal_date'])
                        }}</div>
                    <div class="flex space-x-2 sm:w-1/12">
                        <router-link v-if="permissions.includes('VIEW_PRODUCT')"
    ...
    methods: {
        async getProducts() {
            try {
                ...
                this.product_licence_status = response.data.product_licence_status;
