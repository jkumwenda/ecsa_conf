<template>
  <div class="flex flex-col space-y-4 flex-1">
    ...
    <div v-else class="rounded-2xl bg-catskill-white-100 shadow-sm p-4">
      <form class="flex flex-col sm:w-5/12 w-full space-y-4" @submit.prevent="addEventType" method="POST">
        <label class="block">
...
export default {
  name: "AddEventTypeView",
  components: {
    HeaderView, SpinnerComponent
  },
  data() {
    return {
      headerTitle: "Add event type",
      eventTypeData: {
        event_type: "",
        description: "",
      },
  ...
  methods: {
    async addEventType() {
      this.isLoading = true;
      try {
        const response = await createItem("event_types/", this.eventTypeData);
