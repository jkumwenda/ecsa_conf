<template>
  <div class="flex flex-col space-y-4 flex-1">
    ...
    <div v-else class="rounded-2xl bg-catskill-white-100 shadow-sm p-4">
      <form class="flex flex-col w-5/12 space-y-4" @submit.prevent="addProductLicenceFee" method="POST">
        <label class="block">
          <span class="after:content-['*'] after:ml-0.5 after:text-red-500 block text-sm font-medium text-slate-700">
            Product Licence Fee
          </span>
          <input type="number" name="product_license_fee" v-model="productLicenceFeeData.product_license_fee"
...
export default {
  name: "AddProductLicenceFeeView",
  components: {
    HeaderView, SpinnerComponent
  },
  data() {
    return {
      headerTitle: "Add product licence fee",
  ...
  methods: {
    async addProductLicenceFee() {
      this.isLoading = true;
      try {
        const response = await createItem("product_licence_fees/", this.productLicenceFeeData);
